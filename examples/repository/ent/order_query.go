...
func (oq *OrderQuery) QueryShop() *ShopQuery {
	query := &ShopQuery{config: oq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := oq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (oq *OrderQuery) WithShop(opts ...func(*ShopQuery)) *OrderQuery {
	query := &ShopQuery{config: oq.config}
	for _, opt := range opts {
		opt(query)
	}
...
func (oq *OrderQuery) sqlAll(ctx context.Context) ([]*Order, error) {
	...
	if query := oq.withShop; query != nil {
		ids := make([]int, 0, len(nodes))
		nodeids := make(map[int][]*Order)
		for i := range nodes {
			fk := nodes[i].ShopID
